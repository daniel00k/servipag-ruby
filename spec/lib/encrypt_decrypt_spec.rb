...
describe CryptDecrypt::Encrypt do 
	...
	it "should encrypt and decrypt a string" do
		@encrypted_string = CryptDecrypt::Encrypt.encrypt_using_public_key(@public_key_path, @string)
		CryptDecrypt::Decrypt.decrypt_using_private_key(@private_key_path, @encrypted_string).should eq(@string)
	end
end